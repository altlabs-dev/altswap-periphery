...
import AltFactory from '@uniswap/v2-core/build/AltFactory.json'
...
import AltRouter01 from '../../build/AltRouter01.json'
import AltMigrator from '../../build/AltMigrator.json'
import AltRouter02 from '../../build/AltRouter02.json'
...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  // deploy V2
  const factoryV2 = await deployContract(wallet, AltFactory, [wallet.address])

  // deploy routers
  const router01 = await deployContract(wallet, AltRouter01, [factoryV2.address, WETH.address], overrides)
  const router02 = await deployContract(wallet, AltRouter02, [factoryV2.address, WETH.address], overrides)

  // event emitter for testing
  const routerEventEmitter = await deployContract(wallet, RouterEventEmitter, [])

  // deploy migrator
  const migrator = await deployContract(wallet, AltMigrator, [factoryV1.address, router01.address], overrides)
